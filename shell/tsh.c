...
void eval(char *cmdline) {
  ...
  strcpy(buf, cmdline);
  bg = parseline(buf, argv); // 解析前台后台
  int state = bg ? BG : FG;  // 设置状态为前台或后台
  ...
  if (!builtin_cmd(argv)) { // 如果不是内置命令
    ...
    if (state == FG) {
      ...
      addjob(jobs, pid, state, cmdline); // 添加到作业列表
      Sigprocmask(SIG_SETMASK, &prev_one, NULL);
      waitfg(pid); // 等待前台进程结束
    } else {
      Sigprocmask(SIG_BLOCK, &mask_all, NULL);
...
void do_bgfg(char **argv) {
  ...
  if (argv[1][0] == '%') { // 判断是JID还是PID
    int jid = atoi(
        &argv
            [1]
            [1]); // atio在遇到非数字字符时返回0，此情况在getjobjid中处理会被规避
    if (jid == 0) {
      printf("%s: argument must be a PID or %%jobid\n", argv[0]);
      return;
    }
    struct job_t *job = getjobjid(jobs, jid); // 获取作业
    if (job == NULL) {
